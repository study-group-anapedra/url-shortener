...
class ValidadorAnotacoesTest {
    ...
    void deveTestarRateLimit() throws IllegalAccessException {
        ...
        assertDoesNotThrow(() -> RegraNegocioValidator.validar(objeto, usuarioId));
        assertDoesNotThrow(() -> RegraNegocioValidator.validar(objeto, usuarioId));

        // Terceira tentativa: Deve lançar AbusoDeRequisicaoException
        assertThrows(AbusoDeRequisicaoException.class, () -> {
            RegraNegocioValidator.validar(objeto, usuarioId);
        }, "O Rate Limit deveria ter bloqueado o usuário no 3º acesso.");
    }

    // --- TESTE DA ANOTAÇÃO @ValidarURL ---
    @Test
    @DisplayName("Deve lançar BusinessException quando a URL não seguir o padrão ou tamanho permitido")
    void deveTestarValidacaoDeURL() {
        ...
        assertThrows(BusinessException.class, () -> {
            RegraNegocioValidator.validar(urlRuim, usuarioId);
        ...
        assertThrows(BusinessException.class, () -> {
            RegraNegocioValidator.validar(urlVazia, usuarioId);
        });
    ...
    void deveAceitarUrlValida() {
        ...
        assertDoesNotThrow(() -> RegraNegocioValidator.validar(urlBoa, novoUsuarioId),
